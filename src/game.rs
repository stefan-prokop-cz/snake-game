...
impl Game {
            ...
            if self.has_collided_with_wall() || self.has_bitten_itself() {
                ...
                if let Some(food_point) = self.food {
                    if self.snake.get_head_point() == food_point {
                        ...
                        self.place_food();
                        self.score += 1;
                        let division = (self.width * self.height) / MAX_SPEED;
                        let divisor = if division > 0 { division } else { 1 };
                        if self.score % divisor == 0 {
                            self.speed += 1;
                        }
                    }
                }
    ...
    fn draw_snake(&mut self) {
        ...
        for (i, body) in body_points.iter().enumerate() {
            let prev = if i == 0 { None } else { body_points.get(i - 1) };
            ...
            let symbol = if let Some(&next) = next {
                if let Some(&prev) = prev {
                    if prev.x == next.x {
                        ...
                        if (next == d && prev == r) || (prev == d && next == r) {
                            '╔'
                        } else if (next == d && prev == l) || (prev == d && next == l) {
                            '╗'
                        } else if (next == u && prev == r) || (prev == u && next == r) {
                            '╚'
                        } else {
                            '╝'
                        }
